...
def build_final_scenario_comparison(
    ...
    for label in ["Overall", *ASSETS, *VERTICALS]:
        ...
        # Scenario outcome percentage comes from the existing scenario engine.
        scenario_pct = _num(cell.get("march_pct"))

        # Anchor the scenario amount back to FINAL FY27 Target. This ensures the
        # comparison remains on the management metric base even if source sheets
        # contain small rounding differences.
        scenario_amount = (
        ...
        if current_pct is not None and scenario_pct is not None:
            # Keep as a decimal fraction; the shared "pts" formatter converts
            # it to percentage points exactly once.
...
def render_final_metric_baseline(
    ...
    """Headline management metrics sourced from FINAL, on the single dashboard."""
    ...
    if isinstance(aum, pd.DataFrame) and not aum.empty and "Overall" in aum.index:
        aum_o = aum.loc["Overall"]
        kpi_row([
            ("AUM Target", fmt_cr(aum_o.get("Target")), "FINAL sheet", "off"),
            ("AUM Current", fmt_cr(aum_o.get("Current")),
             fmt_pct(aum_o.get("Achievement %")) if _num(aum_o.get("Achievement %")) is not None else None),
            ("AUM Gap to Target", fmt_cr_signed(-_z(aum_o.get("Gap to Target"))),
             "current minus target", "off"),
            ("Months Done", str(final_metrics.get("months_done", MONTHS_COMPLETED)),
             f"{12 - int(final_metrics.get('months_done', MONTHS_COMPLETED))} months remaining", "off"),
        ])

    st.markdown(
        "<div class='note'>These headline metrics come from the workbook's <b>FINAL</b> sheet. "
        "Current RR = YTD ÷ months completed; Required RR = FY27 Target ÷ 12; "
        "Projected FY = Current RR × 12.</div>",
        unsafe_allow_html=True,
    )
    ...
    with left:
        ...
        if not gs.empty:
            frame = gs.reset_index()[["Metric", *display_cols]]
    ...
    with right:
        ...
        if not ns.empty:
            frame = ns.reset_index()[["Metric", *display_cols]]
            show_table(frame, formats)
        else:
            st.info("Net Sales metrics could not be located in FINAL.")

    if isinstance(aum, pd.DataFrame) and not aum.empty:
        st.markdown("**AUM**")
        aum_display = aum.reset_index().rename(columns={"Metric": "Metric"})
        show_table(
            aum_display,
            {
                "Metric": "txt",
...
def render_final_scenario_comparison(
    ...
    """Put selected scenario directly against the FINAL run-rate metrics."""
    ...
    st.markdown(
        ...
        "The comparison below is anchored to the same FY27 Target, YTD, Current RR and "
        "current-run-rate projection shown above from FINAL.</div>",
        unsafe_allow_html=True,
    )

    left, right = st.columns(2)
    with left:
        st.markdown("**Gross Sales · Current vs Scenario**")
        show_table(gs_frame, gs_formats)
    with right:
        st.markdown("**Net Sales · Current vs Scenario**")
        show_table(ns_frame, ns_formats)

    st.markdown(
        "<div class='note'>Scenario modelling is applied to Overall, Equity, Debt, Liquid, "
        "Retail, DHNI and VRM because those are backed by the detailed RM calculation sheets. "
        "Insti and Digital remain visible in the FINAL current metric grid but are not assigned "
        "a scenario outcome unless detailed calculation data is supplied for them.</div>",
        unsafe_allow_html=True,
    )
